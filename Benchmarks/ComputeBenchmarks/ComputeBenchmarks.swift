...
let benchmarks: @Sendable () -> Void = {
    Benchmark("reference_fanout", configuration: benchmarkConfiguration()) { benchmark in
        let runner = ReferenceFanoutRunner()
        benchmark.startMeasurement()
        for _ in benchmark.scaledIterations {
            blackHole(await runner.runOnce())
        }
        benchmark.stopMeasurement()
    }

    Benchmark("array_filter_512", configuration: benchmarkConfiguration()) { benchmark in
        let runner = ArrayFilterRunner()
        benchmark.startMeasurement()
        for _ in benchmark.scaledIterations {
            blackHole(await runner.runOnce())
        }
    ...
    Benchmark("array_map_512", configuration: benchmarkConfiguration()) { benchmark in
        let runner = ArrayMapRunner()
        benchmark.startMeasurement()
        for _ in benchmark.scaledIterations {
            blackHole(await runner.runOnce())
        }
    ...
    Benchmark("reactive_updates", configuration: benchmarkConfiguration()) { benchmark in
        let runner = await ReactiveUpdatesRunner()
        benchmark.startMeasurement()
        for iteration in benchmark.scaledIterations {
            blackHole(await runner.runOnce(iteration: iteration))
        }
        benchmark.stopMeasurement()
        await runner.cancel()
    }
