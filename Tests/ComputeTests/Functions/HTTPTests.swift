...
struct HTTPTests {
    ...
    @Test func performs_resolved_requests_as_an_async_returns_keyword() async throws {
        ...
        let function = Compute.Keywords.HTTP.Function { request in
            ...
            return Compute.Keywords.HTTP.Response(
    ...
    @Test func recomputes_request_url_when_referenced_url_changes() async throws {
        ...
        let http = Compute.Keywords.HTTP.Function { request in
        ...
        let runtime = try runtime(json, functions: [Compute.Keywords.From.Function(references: references), http])
    ...
    @Test func returns_plain_text_bodies_when_response_is_not_json() async throws {
        let function = Compute.Keywords.HTTP.Function { _ in
            Compute.Keywords.HTTP.Response(data: Data("accepted".utf8), status: 202)
        }
...
private actor HTTPRequestProbe {
    ...
    func response(for request: URLRequest) -> Compute.Keywords.HTTP.Response {
